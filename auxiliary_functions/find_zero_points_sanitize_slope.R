...
#' @param problematic_pixels pixels with slope problem
#' @return a list as explained above
#' 
find_zero_points_sanitize_slope <- function(x, id_date_name="id_date", D_mav_name = "D_mav", problematic_pixels)
{
    ...
    # Vector with id_pixel of pixels with slope problem
    id_pixels_slope_problem <- problematic_pixels[[1]]
    # Vector with right zero of first zero of pixels with slope problem
    right_zero_pixels_slope_problem <- problematic_pixels[[2]]
    ...
    {
        # If the pixel has a slope problem, then account for it
        if(i %in% id_pixels_slope_problem)
        {
            # Get time axis for pixel
            time_axis <- x %>% filter(id_pixel == i, id_date_extended >= right_zero_pixels_slope_problem[which(id_pixels_slope_problem == i)]) %>% select_(id_date_name) %>% pull()
            # Select derivative smoothed with running average (moving average)
            derivative <- x %>% filter(id_pixel == i, id_date_extended >= right_zero_pixels_slope_problem[which(id_pixels_slope_problem == i)]) %>% select_(D_mav_name) %>% pull()
            
        }else
        {
            # If the pixel does not have a slope problem... do as before.
